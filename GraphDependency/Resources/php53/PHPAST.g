...
@after {
  ...
  : ( class_entr_type? CLASS_T IDENTIFIER (EXTENDS_T fully_qualified_class_name)? (IMPLEMENTS_T fully_qualified_class_name_list)?
   ...
   | (TRAIT_T IDENTIFIER LEFT_BRACKET class_statement* RIGHT_BRACKET
    {
        startIndex = ((CommonToken)$TRAIT_T).getStartIndex();
        token = (CommonToken)$RIGHT_BRACKET;
        endIndex = token.getStopIndex();
    }
    
    -> ^(TRAIT_T IDENTIFIER ^(TRAIT_BODY class_statement*)?)
  )
  ;
...
@after {
  ...
  | USE_T fully_qualified_class_name_list trait_adaptations
  {
    token = (CommonToken)$USE_T;
    startIndex = token.getStartIndex();
    token = (CommonToken)$trait_adaptations.stop;
    endIndex = token.getStopIndex();
  }
     -> ^(USE_T fully_qualified_class_name_list trait_adaptations)
  ;
...
@after {
  ...
  : LEFT_BRACKET? trait_adaptation* RIGHT_BRACKET?
  {
...
@after {
  ...
  : fully_qualified_class_name INSTANCEOF_T fully_qualified_class_name_list SEMI_COLON
  {
  ...
  | fully_qualified_class_name AS_T modifier? IDENTIFIER? SEMI_COLON
  {
    token = (CommonToken)$fully_qualified_class_name.start;
    startIndex = token.getStartIndex();
    token = (CommonToken)$SEMI_COLON;
...
@after {
  ...
  | STATIC '::' reference_variable_without_dollar -> ^(VAR_DECL ^('::' STATIC reference_variable_without_dollar ))
//  | fully_qualified_class_name
  ;
...
@init{
  ...
  : HEREDOC_START(id1=IDENTIFIER) 
  {
    ...
    while(true) {
        int index = 0;
        while( index < $id1.text.length() && $id1.text.charAt(index) == input.LA(1) ) {
            heredoc.append((char)input.LA(1) );
            input.consume();
            index++;
            if(index == $id1.text.length()) {
